...
class Test_TC022:
    ...
    def test_phone_textbox(self):
        ...
        if App_CustomerDetails.enter_in_phone_txtbox(self.driver, self.excel_obj, self.itestcaserow):
            log.info("User can add texts in Phone TextBox - " + self.getclassname() + " is PASSED")
        else:
            log.error("User can not add texts in Phone TextBox - " + self.getclassname() + " is FAILED")
