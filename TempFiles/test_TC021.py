...
class Test_TC021:
    ...
    def test_country_ddl(self):
        ...
        if App_CustomerDetails.select_country(self.driver, self.excel_obj, self.itestcaserow):
            log.info("User can select a country - " + self.getclassname() + " is PASSED")
        else:
            log.error("User can not select a country - " + self.getclassname() + " is FAILED")
