...
class Test_TC023:
    ...
    def test_shipping_checkbox(self):
        ...
        if App_CustomerDetails.click_shipping_checkbox(self.driver):
            log.info("User can click on Same Shippin Address as Billing address checkbox - " + self.getclassname() + " is PASSED")
        else:
            log.error("User can not click on Same Shippin Address as Billing address checkbox - " + self.getclassname() + " is FAILED")
