...
class Test_TC018:
    ...
    def test_lastname_textbox(self):
        ...
        if App_CustomerDetails.enter_in_lastname_txtbox(self.driver, self.excel_obj, self.itestcaserow):
            log.info("User can add texts in LastName TextBox - " + self.getclassname() + " is PASSED")
        else:
            log.error("User can not add texts in LastName TextBox - " + self.getclassname() + " is FAILED")
