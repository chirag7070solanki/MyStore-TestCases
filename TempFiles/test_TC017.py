...
from MyStore.appModules import SigIn,App_CustomerDetails, App_ProductSelection, App_VerifyPurchase
...
class Test_TC017:
    ...
    def test_firstname_textbox(self):
        ...
        if App_CustomerDetails.enter_in_firstname_txtbox(self.driver, self.excel_obj, self.itestcaserow):
            log.info("User can add texts in FirstName TextBox - " + self.getclassname() + " is PASSED")
        else:
            log.error("User can not add texts in FirstName TextBox - " + self.getclassname() + " is FAILED")
